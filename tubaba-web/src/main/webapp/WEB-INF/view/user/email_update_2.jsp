...
function clearTip(){
	if($('#emailId').val() != ''){
		...
		if(reg.test($('#emailId').val())){
			if($('#emailId').val() != $('#old_userEmail').val()){
				var flag = false;
				$.ajax({
					type:'get',
					url:'/checkEmail',
				...
				showError('email_tip','不能与原来的邮箱一样',$('#emailId'));
				return false;
			}
		}else{
            ...
            <div class="mod_content p20_4">
                ...
                <div class="phone_edit_content">
                    ...
                    <form id="myForm" action="updateUserEmail" method="post">
                    	...
                    	<input type="hidden" name="old_email" value="${email }" id="old_userEmail" />
                        <div class="phone_box">
                            <input type="hidden" name="newVersion" value="">
                            <p><label class="ml72">新邮箱地址：</label><input type="text" id="emailId" name="email" onblur="clearTip()" class="input_text wd_250" maxlength="64"><span class="tips_warn" id="email_tip"></span></p>
