...
//显示日历
function showCalendar(_year,_month,firstDay) {
    var i = 0,
        monthDay = 0,
        showStr = "",
        _classname = "",
    ...
    td = $("#firstScreenProductIntroduction .product_top_l .calendar-box .caltable tbody td");
    //console.log($($(td).get(index)).find(".calday"))
    //console.log($($(td).get(8)).find(".date"))
    //$($(td).get(8)).find(".date").removeClass("nodate");
    //$($(td).get(8)).find(".date").removeClass("caldisabled");
    //.removeClass(".nodate.caldisabled");
    //显示当前月的天数
    for (let i=1; i<=monthDay; i++) {
        ...
        if(_year === today.getFullYear() && _month === today.getMonth()+1 && i === today.getDate()) {
	        // _classname = "cld-cur"; 
	    	//index = getFirstDay(_year,_month)+1;
	    	//console.log($(td.get(index)));
	    	//$(td.get(index)).find(".calday").append("今天");
	    	//console.log(td);
        	//$(td.get(1)).find(".calday").append("今天");
        	$($(td).get(index)).find(".calday").append("今天");
        } 
        //当日之前的日期（这个判断是因为我有工作需求，就是要求之前的日期不能点击）
            ...
            //_classname = "cld-old";
        	//console.log(i);
        	//$(td.get(1)).find(".calday").append(i);
        	$($(td).get(index)).find(".calday").append(day);
        }
        //其他普通的日期
        else {
        	//console.log(i);
